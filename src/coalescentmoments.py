...
# Theorem 2. Note: Does not include the variance terms (i.e. the diagonals)
# TODO: Add in the variance terms
def sfs_m2(N, G, Peq, Pun, Pne):
    M2 = np.zeros((N, N))
    K = np.arange(N+1)
    for i in range(1,N):
        for j in range(1,i):
...
def fu_moments(n):
    ...
    # Not calculating variance terms
